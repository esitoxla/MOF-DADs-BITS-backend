...
//THIS IS FOR REVENUE PDF GENERATION
...
export function generateRevenuePDF({
  ...
  if (logoPath) {
    try {
      doc.image(logoPath, 450, 30, { width: 80 });
    } catch (err) {
  ...
  doc
    .fontSize(15)
    .text(`IGF Revenue Performance – Quarter ${quarter}, ${year}`, {
      align: "center",
    })
    .moveDown(1);

  doc.fontSize(10).text(`Organization: ${user.organization}`);
  ...
  const columnWidths = [120, 70, 70, 70, 70, 70, 100];

  drawPDFRow(doc, headers, columnWidths, true);
  ...
  grouped.forEach((item) => {
    drawPDFRow(
      ...
      [
        item.category,
        format(item.projection),
        format(item.actual),
        format(item.payment),
        format(item.retention),
        format(item.projectionDec),
        item.remarks || "",
      ],
      columnWidths
    );
  ...
  drawPDFRow(
    ...
    [
      "TOTAL",
      format(totals.projection),
      format(totals.actual),
      format(totals.payment),
      format(totals.retention),
      format(totals.projectionDec),
      "",
    ],
    columnWidths,
    true,
...
// HELPER FUNCTIONS
// ===================================================================
function drawPDFRow(doc, row, widths, bold = false, fill = null) {
  const y = doc.y;
  ...
  row.forEach((text, idx) => {
    const x = 40 + widths.slice(0, idx).reduce((a, b) => a + b, 0);
    doc.text(String(text), x + 3, y + 5, { width: widths[idx] });
  });
